        ...
        >
          ...
          <form @submit.prevent="register_now">
            <v-card-title class="d-flex justify-space-between">
              ...
              <v-btn :nuxt="true" depressed to="/auth/seller/login">Login</v-btn>
            </v-card-title>
            <v-card-subtitle v-if="response_error !== null" class="my-2">
              <v-alert v-for="(errors,index) in response_error" :key="index" class="py-2"
                       type="error" border="left"
              >
                <div class="d-flex">
                  <p class="font-weight-bold">{{ index.split("_").join(" ") | capfirst }}: </p>
                  <p>{{ errors }}</p>
                </div>
              </v-alert>
            </v-card-subtitle>
